...
public class MyData {
    ...
    public void quickSort(int first, int last) {
        ...
        while (lower <= upper) {
            while (lower < upper && data[lower] < pivot) {  //partition
                lower++;
        ...
        //recursion
        quickSort(first, middle-1);  // < pivot
        quickSort(middle+1, last);  // > pivot
    }
    ...
    public void merge(int first, int last) {
        ...
        while (left <= mid) { // data on left side has some item(s) remaining
            result[iAtResult++] = data[left++];
        }
        while (right <= last) { // data on right side has some item(s) remaining
            result[iAtResult++] = data[right++];
        }
    ...
    public void radixSort() {
        ...
        for(d = 1; d <= digits; factor *= 10, d++) {
            ...
            for(int i = 0; i < size; i++) {
                ...
                //if pass = 4  =>  49123  =>  9  (49123/1000 = 49  => 49%10 = 9)
